...
export default function CatalogScreen({ onNavigate }: CatalogScreenProps) {
  ...
  /* LIST */
  return (
    <div className="flex flex-col h-full px-4 pt-1 pb-2 gap-2 animate-fade-in">
      {/* Header row */}
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h2 className="font-golos font-bold text-lg text-[hsl(var(--text-main))]">Каталог</h2>
          <p className="font-golos text-[10px] text-[hsl(var(--text-secondary))]">Профессиональная косметика</p>
        </div>
      ...
      <div className="relative shrink-0">
        ...
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск…" className="w-full pl-9 pr-4 py-2.5 bg-white border border-[hsl(var(--border))] rounded-xl font-golos text-sm focus:outline-none shadow-sm" />
      </div>
      ...
      <div className="flex gap-1.5 overflow-x-auto scrollbar-hide shrink-0">
        {categories.map(cat => (
          <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-xl whitespace-nowrap font-golos text-xs font-semibold transition-all shrink-0 ${activeCategory === cat.id ? "gradient-orange text-white shadow-sm" : "bg-white border border-[hsl(var(--border))] text-[hsl(var(--text-secondary))]"}`}>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide space-y-2 min-h-0">
        {filtered.map((product, i) => {
          ...
          return (
            <div key={product.id} className="bg-white border border-[hsl(var(--border))] rounded-2xl p-3 flex items-center gap-3 shadow-sm animate-fade-in-up" style={{ animationDelay: `${i * 0.04}s` }}>
              {/* Thumb */}
              <button onClick={() => setDetail(product)} className="relative shrink-0">
                <div className="w-14 h-14 rounded-xl overflow-hidden bg-[hsl(var(--gray-soft))]">
                  <img src={product.img} alt={product.name} className="w-full h-full object-cover" />
                </div>
                {product.recommended && (
                  <div className="absolute -top-1 -left-1 w-4 h-4 gradient-orange rounded-full flex items-center justify-center">
                    <Icon name="Star" size={9} className="text-white fill-white" />
                  </div>
                )}
                {isBooked && (
              ...
              {/* Info */}
              <button onClick={() => setDetail(product)} className="flex-1 text-left min-w-0">
                <p className="font-golos text-[10px] text-[hsl(var(--text-secondary))]">{product.brand}</p>
                <p className="font-golos font-semibold text-sm text-[hsl(var(--text-main))] leading-tight truncate">{product.name}</p>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="font-golos font-bold text-sm text-[hsl(var(--text-main))]">{product.price.toLocaleString()} ₽</span>
                  <span className="font-golos text-[10px] text-[hsl(var(--primary))] font-semibold bg-[hsl(var(--orange-light))] px-1.5 py-0.5 rounded-lg">+{product.bonus} Б</span>
                  {!product.inStock && <span className="font-golos text-[10px] text-red-400 font-semibold">Нет</span>}
                </div>
              </button>
              {/* Add / qty */}
              {product.inStock && (
                inCart ? (
                  ...
                  <button onClick={() => addToCart(product)} className="w-8 h-8 bg-[hsl(var(--orange-light))] rounded-xl flex items-center justify-center shrink-0">
        ...
        {/* Bonus hint */}
        <div className="bg-[hsl(var(--orange-light))] border border-[hsl(var(--primary))]/15 rounded-2xl px-4 py-2.5 flex items-center gap-2">
          <Icon name="Sparkles" size={15} className="text-[hsl(var(--primary))] shrink-0" />
          <p className="font-golos text-xs text-[hsl(var(--primary))]">За покупку начисляется <span className="font-bold">+5% от суммы</span> баллами</p>
        </div>
      </div>
